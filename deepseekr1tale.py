...
Blue hue theme, black background, blue text.
"""
...
class DeepSeekTale:
    def __init__(self):
        pygame.init()
        pygame.display.set_caption("DEEPSEEKTALE 0.1")
        ...
        self.dialogue_box_rect = pygame.Rect(40, SCREEN_H - 140, SCREEN_W - 80, 120)
    ...
    def _draw_menu(self):
        ...
        # Border
        make_border_rect(self.screen, BLUE, 6)
        make_border_rect(self.screen, LIGHT_BLUE, 2)

        # Title
        title_y = int(self.menu_title_y)
        title_surf = self.font_title.render("DEEPSEEKTALE", True, BLUE)
        ...
        for i, opt in enumerate(self.menu_options):
            y = SCREEN_H // 2 + i * 50
            color = BLUE if i != self.menu_selection else WHITE
    ...
    def _draw_overworld(self):
        ...
        # Center camera when room is smaller than the screen
        if room_pw <= SCREEN_W:
            ...
            for y in range(room["h"]):
                ...
                if -TILE_SIZE < rx < SCREEN_W + TILE_SIZE and -TILE_SIZE < ry < SCREEN_H + TILE_SIZE:
                    if (x, y) in room["walls"]:
                        pygame.draw.rect(self.screen, (10, 10, 40), (rx, ry, TILE_SIZE, TILE_SIZE))
                        pygame.draw.rect(self.screen, DARK_BLUE, (rx, ry, TILE_SIZE, TILE_SIZE), 1)
                    else:
                        pygame.draw.rect(self.screen, (8, 8, 28), (rx, ry, TILE_SIZE, TILE_SIZE))
        ...
        for x in range(room["w"]):
            for y in range(room["h"]):
                ...
                ry = y * TILE_SIZE - cam_y
                if -TILE_SIZE < rx < SCREEN_W + TILE_SIZE and -TILE_SIZE < ry < SCREEN_H + TILE_SIZE:
                    pygame.draw.rect(self.screen, (15, 15, 40), (rx, ry, TILE_SIZE, TILE_SIZE), 1)
        ...
        # Room name
        blit_text(self.screen, room["name"], (SCREEN_W // 2, 10), BLUE, 16, "center")
    ...
    def _draw_dialogue(self):
        ...
        if self.dialogue_is_battle:
            ...
            # Draw overworld behind dialogue
        ...
        pygame.draw.rect(self.screen, DARK_BLUE, inner, 1)
        ...
        # Name plate
        name_rect = pygame.Rect(self.dialogue_box_rect.x + 10, self.dialogue_box_rect.y - 8, 60, 20)
        draws_box(self.screen, name_rect, BLUE, BLACK, 2)
        blit_text(self.screen, "*", (name_rect.x + 5, name_rect.y + 2), BLUE, 16)

        # Arrow
        if self.dialogue_show_arrow:
            ax = self.dialogue_box_rect.right - 20
            ay = self.dialogue_box_rect.bottom - 20
            self._draw_heart(ax, ay, BLUE, 4)
    ...
    def _draw_battle(self):
        ...
        # Battle box (where soul moves)
        draws_box(self.screen, self.battle_box, BLUE, (5, 5, 25), 3)

        # Soul (heart) — always visible
        ...
        # Enemy info
        if self.enemy_data:
            en = self.enemy_data["name"]
            blit_text(self.screen, en, (30, 30), WHITE, 28)

            # Enemy HP bar
            bar_rect = pygame.Rect(30, 60, 200, 20)
            draws_box(self.screen, bar_rect, BLUE, (10, 10, 30), 2)
            if self.enemy_max_hp > 0:
                fill_w = int(200 * self.enemy_hp / self.enemy_max_hp)
                hp_rect = pygame.Rect(31, 61, fill_w, 18)
                pygame.draw.rect(self.screen, YELLOW, hp_rect)
            hp_text = f"{self.enemy_hp}/{self.enemy_max_hp}"
            blit_text(self.screen, hp_text, (230, 63), WHITE, 16)

        # Player info
        player_info = f"LV {self.player.lv}  HP {self.player.hp}/{self.player.max_hp}"
        blit_text(self.screen, player_info, (SCREEN_W - 20, 20), GREEN, 24, "topright")

        # HP bar (player)
        hp_bar = pygame.Rect(SCREEN_W - 220, 45, 200, 16)
        draws_box(self.screen, hp_bar, BLUE, (10, 10, 30), 2)
        if self.player.max_hp > 0:
            fill_w = int(200 * self.player.hp / self.player.max_hp)
            hp_fill = pygame.Rect(SCREEN_W - 219, 46, fill_w, 14)
            color = RED if self.player.hp < self.player.max_hp // 4 else YELLOW
            pygame.draw.rect(self.screen, color, hp_fill)
        blit_text(self.screen, f"HP", (SCREEN_W - 240, 45), GREEN, 16)
        ...
        # Battle text (timer is decremented in update, not draw)
        if self.battle_text_timer > 0 and self.battle_text:
            blit_text(self.screen, self.battle_text, (SCREEN_W // 2, SCREEN_H - 30), WHITE, 22, "center")
    ...
    def _draw_battle_message(self):
        ...
        if self.battle_timer // 30 % 2 == 0:
            blit_text(self.screen, "[Press Z to continue]", (msg_rect.centerx, msg_rect.bottom - 10),
                      LIGHT_BLUE, 16, "center")

    def _draw_battle_background(self):
        # Background gradient effect
        for i in range(SCREEN_H):
            c = max(0, min(30, 10 + int(math.sin(i * 0.01) * 8)))
            pygame.draw.line(self.screen, (c, c, c + 15), (0, i), (SCREEN_W, i))
        # Grid pattern
        for x in range(0, SCREEN_W, 40):
            pygame.draw.line(self.screen, (15, 15, 35), (x, 0), (x, SCREEN_H))
        for y in range(0, SCREEN_H, 40):
            pygame.draw.line(self.screen, (15, 15, 35), (0, y), (SCREEN_W, y))

    def _draw_battle_menu(self):
        menu_rect = pygame.Rect(20, SCREEN_H - 120, SCREEN_W - 40, 100)
        draws_box(self.screen, menu_rect, BLUE, BLACK, 3)

        if self.battle_phase == "menu":
            cols = 2
            for i, opt in enumerate(self.battle_menu_options):
                x = 40 + (i % cols) * 150
                y = SCREEN_H - 100 + (i // cols) * 35
                color = WHITE if i == self.battle_menu_selection else BLUE
                if i == self.battle_menu_selection:
                    self._draw_heart(x - 12, y + 8, BLUE, 4)
                blit_text(self.screen, opt, (x, y), color, 24)

        elif self.battle_phase == "act":
            for i, act in enumerate(self.battle_act_options):
                x = 40
                y = SCREEN_H - 100 + i * 30
                color = WHITE if i == self.battle_act_selection else BLUE
                if i == self.battle_act_selection:
                    self._draw_heart(x - 12, y + 8, BLUE, 4)
                blit_text(self.screen, act, (x, y), color, 22)
            ...
            if items:
                for i, item in enumerate(items):
                    x = 40
                    y = SCREEN_H - 100 + i * 30
                    color = WHITE if i == self.battle_item_selection else BLUE
                    if i == self.battle_item_selection:
                        self._draw_heart(x - 12, y + 8, BLUE, 4)
                    count = self.player.item_counts.get(item, 0)
                    blit_text(self.screen, f"{item} x{count}", (x, y), color, 22)
            else:
                blit_text(self.screen, "(No items)", (40, SCREEN_H - 90), (100, 100, 100), 22)

        elif self.battle_phase == "mercy":
            for i, opt in enumerate(self.battle_mercy_options):
                x = 40 + i * 150
                y = SCREEN_H - 90
                color = WHITE if i == self.battle_mercy_selection else BLUE
            ...
            blit_text(self.screen, prog_text, (40, SCREEN_H - 55), (100, 150, 255), 18)
    ...
    def _draw_fight(self):
        ...
        blit_text(self.screen, "Press Z to attack!",
                  (SCREEN_W // 2, SCREEN_H - 30), WHITE, 22, "center")
        ...
        if self.enemy_data:
            ...
            blit_text(self.screen, hp_text, (30, 60), YELLOW, 22)
    ...
    def _draw_pause(self):
        ...
        blit_text(self.screen, "DEEPSEEKTALE 0.1", (SCREEN_W // 2, 130), LIGHT_BLUE, 24, "center")
